...
void PrivateInstantDataFeed<T>::Init(const DataFeedDesc& data_feed_desc) {
  ...
  PADDLE_ENFORCE(data_feed_desc.has_multi_slot_desc(), "Multi_slot_desc has not been set.");
...
bool MultiSlotFileInstantDataFeed::ParseOneMiniBatch() {
  ...
  while (batch_size_ < default_batch_size_ && offset_ < end_) {
    ...
    ++batch_size_;
		// OPTIMIZE: It is better to insert check codes between instances for format checking
  }
